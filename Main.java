	...
	public static void oyuncuyaSoruSorIlk(Oyuncu oyuncu) throws IOException {
		...
		if ((secim.equals("böl") || secim.equals("bol") || secim.equals("BOL") || secim.equals("BÖL")
			...
			for (int i = 1; i <= 2; i++) { // iki kere yazdırıyorsa sorun burada.
				...
				if (i == 1) {
					try {
						if (Integer.parseInt(oyuncu.oyuncuSetleriniHesapla(oyuncu.getBolunenOyuncuKartlari1())) > 21) {
				...
				if (i == 2) {
					try {
						if (Integer.parseInt(oyuncu.oyuncuSetleriniHesapla(oyuncu.getBolunenOyuncuKartlari2())) > 21) {
							System.out.print(" PATLADIN :) \n");
							oyuncu.setBolunenSetPasaBastiMi(true, i - 1);
